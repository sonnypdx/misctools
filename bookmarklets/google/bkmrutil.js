...
(function (){
var l="title,h1,h2,h3,h4,h5,h6".split(','),
...
gtTtl=function(){
var ts=l.reduce((g,x)=>g.concat(gSS(x)),[]).sort((a,b)=>b.length-a.length);
return ts.length?ts[0]:d.title;
},
...
gWs=function(ht){
return gSS(ht).reduce((g,x)=>g.concat(x.toLowerCase().split(' ').filter(s=>s.length>3&&STWD.indexOf(s)<0)),[]);
},
// get the keywords list from the heading tags of the given webpage while bookmarking
gtKwH=function(){
var d1=l.reduce((g,t)=>g.concat(gWs(t)), []).reduce((g,x)=>g.set(x,(g.get(x)||0)+1), new Map());
